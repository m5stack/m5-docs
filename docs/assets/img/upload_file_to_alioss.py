...
if __name__=='__main__':
    ...
    for root, dirs, files in os.walk(os.getcwd()):
        for file in files:
            if file == 'upload_file_to_alioss.py' or file == 'temp.md':
                pass
            else:
                filename = os.path.join(root, file)
                remote_filepath = os.path.relpath(filename).replace('\\', '/')
                # print(filename)   
                print('docs/en/'+remote_filepath)
                bucket.put_object_from_file('docs/ja/'+remote_filepath, filename)
